...
@Component
public class AdminVendorApprovalDelivery implements VendorApprovalDelivery {
    private final RestClient client;
    private final TokenService tokens;

    public AdminVendorApprovalDelivery(RestClient.Builder builder,
                                       TokenService tokens,
    ...
    @Override
    public void deliver(String identifier, String companyName, String token, Instant expiresAt) {
        client.post()
            .uri("/internal/v1/notifications/vendor-approved")
            .header(HttpHeaders.AUTHORIZATION,
                "Bearer " + tokens.issueInternalServiceToken("authentication-service", Instant.now()))
            .body(new DeliveryRequest(identifier, companyName, token, expiresAt))
            .retrieve()
            .toBodilessEntity();
    }

    private record DeliveryRequest(String identifier, String companyName,
